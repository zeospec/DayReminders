// Login Page Logic
import { signInWithGoogle, getCurrentUser, auth } from './auth.js';

const googleSignInBtn = document.getElementById('googleSignInBtn');
const loginError = document.getElementById('loginError');
const loginErrorMessage = document.getElementById('loginErrorMessage');

if (!googleSignInBtn) {
    console.error('Google sign in button not found!');
} else {
}

// Handle Google Sign In
googleSignInBtn.addEventListener('click', async () => {
    try {
        // Hide any previous errors
        loginError.classList.add('hidden');
        
        // Disable button during sign in
        googleSignInBtn.disabled = true;
        googleSignInBtn.innerHTML = `
            <span class="material-symbols-outlined animate-spin">refresh</span>
            <span>Signing in...</span>
        `;
        
        // Sign in with Google
        const user = await signInWithGoogle();
        
        // Verify the user is actually set in auth
        const verifyUser = getCurrentUser();
        if (!verifyUser) {
            console.error('⚠️ User is null immediately after sign in!');
            throw new Error('Authentication failed - user not set');
        }
        
        // Set a flag in sessionStorage to indicate we're coming from login
        sessionStorage.setItem('fromLogin', 'true');
        sessionStorage.setItem('loginTime', Date.now().toString());
        
        // Wait for auth state to be fully persisted
        await new Promise(resolve => setTimeout(resolve, 500));
        
        // Double-check auth state before redirecting
        const finalCheck = getCurrentUser();
        
        if (!finalCheck) {
            console.error('⚠️ User lost after wait! This indicates a persistence issue.');
            throw new Error('Authentication state not persisted');
        }
        
        // Use location.replace instead of href to avoid back button issues
        window.location.replace('index.html');
    } catch (error) {
        console.error('Sign in error:', error);
        
        // Show error message
        loginErrorMessage.textContent = error.message || 'Failed to sign in. Please try again.';
        loginError.classList.remove('hidden');
        
        // Re-enable button
        googleSignInBtn.disabled = false;
        googleSignInBtn.innerHTML = `
            <svg class="w-6 h-6" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            <span>Continue with Google</span>
        `;
    }
});
